...
pub fn exec(cmd: &str) -> XCode
{
	...
	loop
	{
		...
		if fatarrow
		{
			...
			let path = args[i + 1];
			if crate::fs::reopen(path, lefthandle).is_err()
			{
	...
	let res = match args[0]
	{
		...
		cmd =>
		{
			if crate::sys::proc::spawn(cmd).is_ok()
			{
				XCode::CMD_SUCCESS
			}
			else
	...
	if redir
	{
		for i in 0..3
		{
			crate::fs::reopen("/dev/console", i).ok();
		}
	}
...
pub fn main(args: &[&str]) -> XCode
{
	match args.len()
	{
		...
		{
			let pname = args[1];
			if let Ok(contents) = crate::fs::read_to_str(pname)
			{
...
// Run
pub fn run() -> crate::user::shell::XCode
{
	println!();

	let mut prompt = crate::sys::prompt::Prompt::new();
...
pub fn shcomp(ln: &str) -> Vec<String>
{
	...
	{
		let pname = crate::fs::rpath(args[i]);
		let dname = crate::fs::dname(&pname);
		let fname = crate::fs::fname(&pname);
		...
		if let Some(directory) = crate::fs::directory::Directory::open(dname)
		{
